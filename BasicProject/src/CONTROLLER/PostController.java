	...
	public Command postList() {
		...
		while (true) {
			...
			for (PostVo post : noticePosts) {
				...
				String content = String.format("%-2d | %10s#공지사항 : %-50s%s  ", post.getPost_id(),
						ANSI_BOLD + ANSI_LIGHT_RED, // 공지사항을 빨간색 볼드체로 출력
						post.getTitle(), ANSI_RESET // 색상 및 볼드체 초기화
				);
				System.out.println(ANSI_BOLD + ANSI_LIGHT_RED + "+" + "-".repeat(width - 2) + "+" + ANSI_RESET); // 공지사항
																													// 테두리
																													// 빨간색
				System.out.printf(ANSI_BOLD + ANSI_LIGHT_RED + "| %-" + (width - 3) + "s\n" + ANSI_RESET, content); // 공지사항
																													// 내용
				System.out.println(ANSI_BOLD + ANSI_LIGHT_RED + "+" + "-".repeat(width - 2) + "+" + ANSI_RESET); // 테두리
																													// 닫기
			}
